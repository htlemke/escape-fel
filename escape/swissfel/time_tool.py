...
def TTextract(
    ...
    profiles = ixppy.TTextractProfiles(
        det,
        refmon=refmon,
        refthreshold=refthreshold,
        lmon=lmon,
        lmonthreshold=lmonthreshold,
        Nmax=30,
        calibcycles=0,
        append_to_det=False,
        saveoffs=saveoffs,
...
def teach_filter(profiles,):
    nfh = plt.figure("Digital filter design: Select signal")
    plt.clf()
    p = profiles[2, :]
    plt.plot(p)
    siglim = np.round(plot_utils.getSpanCoordinates("horizontal")).astype(int)
    ...
    nfh = plt.figure("Digital filter design: Select noise region")
    plt.clf()
    plt.plot(profiles.transpose())
    print("select lower limit of noise area (NB: has same width as signal range!)")
    noiselim = plt.ginput(1)
    noiselim = round(noiselim[0][0]) + np.array([0, np.diff(np.array(siglim))[0]])
    plt.axvspan(noiselim[0], noiselim[1], facecolor="r", alpha=0.5)
    plt.axvline(noiselim[0], color="r")
    plt.axvline(noiselim[1], color="r")
    print(noiselim)
    nacl = []
    for p in profiles:
...
def TTextractFilterPositions(Areadet, filtsettings=None, polysettings=None):
    ...
    for ds in Areadet.TTtraces:
        ...
        data = ds[:]
        if polysettings:
            tpos, tamp, tppos, tpcen = TTapplyFilter(
                data, filtsettings, polysettings=polysettings
            )
        else:
...
def extractProfiles(
  ...
  """
    ...
    proflimits, profile = ixppy.getProfileLimits(
        dat, lims=profileLimits, transpose=transpose
    )
...
def extractProfilesCorr(
    Areadet,
    xrayoff=None,
    laseroff=None,
    dataset_name_traces="TTtraces",
    dataset_name_traces_raw="TTtraces_raw",
    evaluate=False,
):
    """
    Areadet is the dataset with the camera images
...
def extractProfilesOnly(
    Areadet,
    profileLimits=None,
    transpose=False,
    dataset_name_traces="TTtraces_raw",
    force=True,
):
    """ 
    Areadet is the dataset with the camera images
...
def extractFromRunList(
    ...
    for run in runlist:
        ...
        extractFromRun(
            ...
            xrayoffCode=xrayoffCode,
            laseroffCode=laseroffCode,
            filter=filter,
            save=save,
        )
        logbook("done!")
...
def applyFilterToAll(det, filter=None, kind="stepUp"):
    ...
    traces = det.TTtraces
    o = ixppy.applyFunction(
        applyFilter,
        [traces, filter],
        dict(kind=kind),
        outputtypes=["memdata"] * 3,
        forceCalculation=True,
        transposeStack=False,
    )
    det["TTpos"] = o[0]
    det["TTamp"] = o[1]
    det["TTfwhm"] = o[2]
