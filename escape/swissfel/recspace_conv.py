...
class THC_robot:
    ...
    def plot_geom(self,mu=None,eta=None, phi=None, gamma=None, delta=None, detector_distance=None, energy= 8000, ub_matrix=None,fig="Recspace vis", ax=None):
        ...
        pxpos = deque(self.qconv.area(mu,eta,90,phi,gamma,delta,en=energy,UB=ub_matrix))
        pxpos.rotate(0)
        orpos = deque(self.qconv.point(mu,eta,90,phi,gamma,delta,en=energy,UB=ub_matrix))
        orpos.rotate(0)
        if ax is None:
        ...
        ret = ax.plot_surface(*pxpos,alpha=.6,
                            # facecolors=fcolors,
                            # vmin=minn,vmax=maxx.
                            )
        ...
        arrowlength = .3 * np.max([np.diff(ax.get_xlim()),np.diff(ax.get_ylim()),np.diff(ax.get_zlim())])
        ax.quiver(*orpos,0,0,arrowlength,color='b',arrow_length_ratio=.2)
        ax.quiver(*orpos,0,arrowlength,0,color='g',arrow_length_ratio=.2)
        ax.quiver(*orpos,arrowlength,0,0,color='r',arrow_length_ratio=.2)

        ax.axis('equal')

    def get_px_hkl(self,mu=None,eta=None, phi=None, gamma=None, delta=None, detector_distance=None, energy= 8000, ub_matrix=None):
        pixsz_plot = self.pixsz*80
        det_sz_px_plot = np.round(self.det_sz_px *self.pixsz /pixsz_plot).astype(int)
        ...
        self.qconv.init_area(
            ...
            *self.det_sz_px[::-1],
            distance=detector_distance,
            pwidth1=self.pixsz,pwidth2=self.pixsz)
