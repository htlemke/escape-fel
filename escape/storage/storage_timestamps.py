...
class ArrayTimestamps:
    ...
    ``ArrayTimestamps`` stores raw measurement data together with a
    monotonically increasing timestamp array. It constructs a
    ``ScanTimestamps`` instance for grouped interval-based selection and
    optional grid metadata handling.

    Args:
        data: Numeric data array.
        timestamps: Timestamp values aligned with ``data``.
        timestamp_intervals: Optional list of interval boundaries defining scan steps.
        parameter: Optional metadata for each timestamp interval.
...
_ARRAY_TIMESTAMPS_NP_METHODS = [
    "mean", "std", "min", "max", "sum", "median", "average",
...
class ScanTimestamps:
    """Scan metadata layer for timestamp-indexed arrays.

    ``ScanTimestamps`` partitions an ``ArrayTimestamps`` dataset into
    steps defined by timestamp intervals. It exposes step-wise parameter
    metadata and, when ``grid_specs`` is provided, constructs a ``Grid``
    mapping scans onto an N-D grid layout.

    Args:
        parameter: Metadata describing step parameters and values.
        timestamp_intervals: List of timestamp intervals for each scan step.
        array: Underlying ``ArrayTimestamps`` instance.
    ...
    # Step-delegation methods (nanmean, mean, …, any, all) are injected
    # below the class definition via _SCAN_TIMESTAMPS_STEP_DELEGATE.
    ...
    def _load_from_h5(group):
        ...
        timestamp_intervals = group["scan"]["timestamp_intervals"][()]
        parameter = {}
        for parname, pargroup in group["scan"]["parameter"].items():
            values = pargroup["values"]
            if not len(values) == len(timestamp_intervals):
                raise Exception(
        ...
        if "grid_specs" in group["scan"].keys():
            try:
                ...
                grid_specs = None
        return parameter, timestamp_intervals, grid_specs
...
# ---------------------------------------------------------------------------
# Programmatic step-delegation method injection for ScanTimestamps
# ---------------------------------------------------------------------------
# Mirrors the _SCAN_STEP_DELEGATE / _make_scan_step_method pattern in storage.py.
# Every name here must also appear in _ARRAY_TIMESTAMPS_NP_METHODS above.
_SCAN_TIMESTAMPS_STEP_DELEGATE = [
    "mean", "std", "min", "max", "sum", "median", "average",
    "nanmean", "nanstd", "nanmin", "nanmax", "nansum", "nanmedian",
    "any", "all",
]
...
def _make_scan_ts_step_method(name):
    """Factory: build a ScanTimestamps method that applies ArrayTimestamps.{name} per step."""
    np_func = getattr(np, name, None)
    np_summary = next(
        (line.strip() for line in (np_func.__doc__ or "").split("\n") if line.strip()), ""
    ) if np_func else ""
    doc = (
        f"Apply :meth:`ArrayTimestamps.{name}` to each scan step.\n\n"
        + (f"{np_summary}\n\n" if np_summary else "")
    ...
    method.__qualname__ = f"ScanTimestamps.{name}"
    method.__doc__ = doc
    return method


for _name in _SCAN_TIMESTAMPS_STEP_DELEGATE:
    setattr(ScanTimestamps, _name, _make_scan_ts_step_method(_name))
del _name
