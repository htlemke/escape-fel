...
class Array:
    ...
    def digitize(self, bins, right=False):
        try:
            bn = np.digitize(self.data, bins, right=right)
            ix = bn.argsort(kind="mergesort")
            if right:
                side = "right"
            else:
                side = "left"
            sn = np.sign(np.diff(bins[:2]))[0]
            scan = Scan(
                parameter_names=[
                    self.name + "_binned_min_" + side,
                    self.name + "_binned_max_" + side,
                ],
                values=[np.insert(bins, sn * np.inf, 0), np.append(bins, -sn * np.inf)],
            )
            return Array(
                data=self.data.take(ix, axis=self.eventDim),
                eventIds=self.eventIds[ix],
                stepLengths=np.bincount(ix),
                scan=scan,
                eventDim=self.eventDim,
            )
        except TypeError:
            return self.digitize(self.compute())

    def filter(self):
        pass

    def compute(self):
        """In case of dask.Array data (data has compute method): Computes the 
        Array data and returns a new in-memory version of the escape Array instance"""
        if hasattr(self.data, "compute"):
            ...
            raise TypeError("The data field does not have a compute method!")
