...
auto-starts a :class:`~escape.stream.Stream`; no explicit
...
class StreamSession:
    ...
    Parameters
    ----------
    source : str, EventWorker, or EventHandler
        ...
        - any ``DataHubEventHandler``, ``DataHubLocalEventHandler``,
          ``MultiSourceEventHandler``, or ``EventHandler_SFEL``/``LocalEventHandler``
          instance
        - an already-constructed ``EventWorker`` — used as-is
        - ``None`` — uses the module default ``EventWorker`` if one is registered,
          otherwise creates a ``DataHubEventHandler(backend='bsread')``

        When ``psi-datahub`` is not installed, the ``'host:port'``/``'hostname'``
        and ``'bsread'``/``'auto'`` forms transparently fall back to the plain
        (non-datahub) ``LocalEventHandler``/``EventHandler_SFEL`` — same result,
        no extra dependency. ``'redis'`` has no such fallback and always requires
        ``psi-datahub``.
    ...
    **handler_kwargs
    ...
    def __init__(self, source=None, auto_accumulate=True, make_default=True, **handler_kwargs):
        from .escape_stream import EventWorker
        ...
        if source is None:
        ...
        elif isinstance(source, EventWorker):
            # Already-constructed EventWorker — use as-is.
            ew = source
            ...
            if source == "redis":
                # No non-datahub redis path exists; this backend hard-requires psi-datahub.
                if not _HAS_DATAHUB:
                    raise RuntimeError("psi-datahub not installed; cannot use the 'redis' backend.")
                ew = EventWorker(DataHubEventHandler(backend="redis", **handler_kwargs), make_default=make_default)
            elif source in ("bsread", "auto"):
                # Both ultimately mean "bsread via the dispatcher" when datahub is
                # unavailable -- fall back to the plain (non-datahub) SFEL handler.
                if _HAS_DATAHUB:
                    ew = EventWorker(DataHubEventHandler(backend=source, **handler_kwargs), make_default=make_default)
                else:
                    ew = EventWorker(EventHandler_SFEL(**handler_kwargs), make_default=make_default)
            else:
                # Parse 'host:port' or bare 'hostname' -- a local/direct bsread sender.
                if ":" in source:
                ...
                if _HAS_DATAHUB:
                    ...
                    # Fall back to the plain bsread-only handler -- same result,
                    # no psi-datahub dependency.
                    ew = EventWorker(
                        LocalEventHandler(host=host, port=port, **handler_kwargs),
                        make_default=make_default,
                    )
        else:
    ...
    def _get_or_create(self, name):
        ...
        if name in streams:
            return streams[name]
        from .escape_stream import Stream
...
def gather(*streams, seconds=0, n_events=None):
    ...
    ----------
    *streams : Stream
        One or more :class:`~escape.stream.Stream` objects.
    seconds : float
